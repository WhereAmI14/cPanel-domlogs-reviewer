...
Usage:
  ...
  Remote bootstrap execution:
    curl -fsSL https://raw.githubusercontent.com/WhereAmI14/cPanel-domlogs-reviewer/dev/logs-reviewer-hybrid.sh | bash
...
run_local_bundle() {
  local script_path script_dir bundle_dir
  script_path="${BASH_SOURCE[0]:-}"
  [[ -n "$script_path" && -f "$script_path" ]] || return 1

  script_dir="$(cd "$(dirname "$script_path")" && pwd)"
  bundle_dir="$script_dir/hybrid"
  [[ -f "$bundle_dir/runner.sh" && -f "$bundle_dir/log_enrich.py" ]] || return 1

  cp "$bundle_dir/runner.sh" "$TMPDIR/runner.sh"
  cp "$bundle_dir/log_enrich.py" "$TMPDIR/log_enrich.py"
  cp "$bundle_dir/README.md" "$TMPDIR/README.md" 2>/dev/null || true
